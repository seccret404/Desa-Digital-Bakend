...
class Profil{
     ...
     static findById(id, result) {
      ...
      static update(id, profil, result) {
          db.query(
              "UPDATE profiles SET nama_desa = ?, alamat_kantor = ?, kecamatan = ?, provinsi = ?, profil_singkat = ?, gambar_desa = ?, batas_barat = ?, batas_timur = ?, batas_utara = ?, batas_selatan = ?, visi_desa = ?, misi_desa = ?, sejarah_desa = ? WHERE id = ?",
              [profil.nama_desa, profil.alamat_kantor, profil.kecamatan, profil.provinsi, profil.profil_singkat, profil.gambar_desa, profil.batas_barat, profil.batas_timur, profil.batas_utara, profil.batas_selatan, profil.visi_desa, profil.misi_desa, profil.sejarah_desa, id],
              (err, res) => {
                  if (err) {
                      result(err, null);
                      return;
                  ...
                  if (res.affectedRows == 0) {
                      // profil tidak ditemukan dengan id yang diberikan
                      result({ kind: "not_found" }, null);
                      return;
                  }
      
                  console.log("profil updated: ", { id: id, ...profil });
